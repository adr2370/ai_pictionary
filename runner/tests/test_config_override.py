"""Verify that main_py_python overrides sys.executable when set."""
import json
import sys
...
def test_main_py_python_override_is_used(tmp_path):
    ...
    (workdir / "main.py").write_text(
        "import sys\n"
        f"open(r'{str(marker).replace(chr(92), '/')}', 'w').write(sys.executable)\n"
    ...
    app = build_app(cfg_path, python_executable="definitely-not-a-real-python")
